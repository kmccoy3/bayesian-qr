...
for (i in 1:I){
  for (j in 1:J){
    err <- rnorm(n=1,mean=0,sd=sqrt(1.0)) #!!! ERROR DISTRIBUTION
...
S <- 70000
no.params <- 2
sig2.beta <- 0.0065          # Used to control acceptance rate of betas!! Need to adjust for different taus!
sig2.alpha <- 0.13           # Used to control acceptance rate of alphas!! Need to adjust for different taus!
B0 <- 100*diag(c(1,1))
c0<-0.01; d0<-0.01
k0<-0.01; w0<-0.01
...
betas.1 <- mvrnorm(n=1,mu=c(0,0),Sigma=diag(c(1,1)))    # Indices: 1-2
...
for (s in 2:S){
  ...
  num <- as.vector(-t(betas.p)%*%solve(B0)%*%betas.p/2)-(double.summer(betas.p,ALPHAS.0[s-1,],ALPHAS.1[s-1,])/PARAMS[s-1,3])
  den <- as.vector(-t(betas.c)%*%solve(B0)%*%betas.c/2)-(double.summer(betas.c,ALPHAS.0[s-1,],ALPHAS.1[s-1,])/PARAMS[s-1,3]) 
  r <- exp(num-den)
  ...
  for (i in 1:I){
    ...
    num <- as.vector(-t(alphas.p)%*%alphas.p/(2*PARAMS[s-1,4])) - (single.summer(betas.c,alphas.p,i)/PARAMS[s-1,3])
    den <- as.vector(-t(alphas.c)%*%alphas.c/(2*PARAMS[s-1,4])) - (single.summer(betas.c,alphas.c,i)/PARAMS[s-1,3])
    r <- exp(num-den)
...
plot(PARAMS[5000:S,4],type='l')
betas.accept/S
alphas.accept/S/I

acf(PARAMS[1:S,1], lag.max=100)
acf(PARAMS[seq(1,S,5),1], lag.max=100)
hist(PARAMS[1:S,2], breaks="scott")

hist(PARAMS[seq(5000,S,5),1], breaks="scott")
hist(PARAMS[seq(5000,S,5),2], breaks="scott")

mean(PARAMS[5000:S,1])
mean(PARAMS[5000:S,2])

mean(PARAMS[seq(5000,S,5),1])
mean(PARAMS[seq(5000,S,5),2])
